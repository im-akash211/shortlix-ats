...
import { TrendingUp, Users, CheckSquare, Calendar, ChevronRight, Layers, Award, Clock, ThumbsUp } from 'lucide-react';
...
export default function Dashboard({ user }) {
  ...
  const summaryParams = { status: jobStatus === 'all' ? '' : jobStatus };
  ...
  const { data: pendingActions } = useQuery({
    queryKey: ['dashboard', 'pendingActions'],
  ...
  const offerJoinRate = totalOffered > 0 ? Math.round((totalJoined / totalOffered) * 100) : 0;

  const funnelStages = [
    { label: 'Applied',     count: Number(getMetric('Applied')?.value  ?? 0) },
    { label: 'Shortlisted', count: Number(shortlistedMetric?.value     ?? 0) },
    { label: 'Interview',   count: Number(getMetric('Interview')?.value ?? 0) },
    { label: 'Offered',     count: totalOffered },
    ...
    <div className="h-full overflow-y-auto" style={{ fontFamily: 'Inter, sans-serif', backgroundColor: '#f8f9fa' }}>
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-sm text-[#565e72] mt-0.5">Track your hiring pipeline at a glance</p>
        </div>
        <div className="flex items-center gap-1 bg-white rounded-lg p-1" style={{ boxShadow: '0 12px 32px rgba(25,28,29,0.06)' }}>
          {STATUS_OPTIONS.map((s) => (
            <button
              key={s}
              onClick={() => setJobStatus(s)}
              className={`px-3 py-1.5 rounded text-xs font-semibold transition-all capitalize ${
                jobStatus === s ? 'bg-[#0058be] text-white' : 'text-[#424754] hover:bg-[#f3f4f5]'
              }`}
            >
              {s}
            </button>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-xl" style={{ boxShadow: '0 12px 32px rgba(25,28,29,0.06)' }}>
              <p className="text-[#565e72] text-xs font-bold uppercase tracking-wider mb-4">Offered</p>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded flex items-center justify-center bg-[#dae2fa]">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#565e72]">Total offers made</p>
                </div>
              </div>
            </div>
